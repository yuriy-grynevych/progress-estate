...
export async function POST(req: NextRequest) {
  ...
  const prompt = `Ти — досвідчений рієлтор. Напиши короткий рекламний опис нерухомості українською мовою на основі параметрів нижче.

Параметри:
${paramText}

Вимоги до формату — поверни ТІЛЬКИ HTML, без пояснень:
1. Перший абзац — коротке (1-2 речення) привабливе вступне речення без зайвих емоджі
2. Потім <ul> з <li> для кожного ключового параметру у форматі "Назва: значення"
3. Останній абзац — коротке заключне речення про переваги або заклик до дії
4. Не додавай заголовки <h1>/<h2>, лише <p> і <ul><li>
5. Текст лаконічний, без води`;
