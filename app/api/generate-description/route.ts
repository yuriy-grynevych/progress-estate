...
export async function POST(req: NextRequest) {
  ...
  const rawExisting: string = params.existingDescription ?? "";
  const existingText = rawExisting
    .replace(/<[^>]+>/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  ...
  const existingBlock = existingText
    ? `\nДодатковий опис від ріелтора (використовуй всі деталі звідси):\n${existingText}\n`
    : "";

  const prompt = `Ти — рієлтор-копірайтер. Напиши опис нерухомості українською мовою.

Структурні дані об'єкта:
${paramText}
${existingBlock}
...
2. <ul> з <li> — перелич ВСІ характеристики: спочатку структурні дані, потім деталі з додаткового опису (ремонт, інфраструктура, особливості тощо). Без emoji. Формат: "Площа: 92 м²", "Котел: конденсаційний двоконтурний", тощо.

Заборонено:
- Емодзі будь-які
- Вигадані факти яких немає ні в структурних даних, ні в додатковому описі
